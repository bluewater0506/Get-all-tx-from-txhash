  ...
  const covertDecimal = (amount, decimal, dir) => {
    ...
    if (dir == 'toBN') {
      ...
      if (arr[1] && arr[1] != '') {
        ...
        return removeHeadZero(arr[0] + decimal_str);
      }
    }
    else {
      if (strAmount.length >= decimal) {
        var top = strAmount.substr(0, strAmount.length - decimal);
        var low = strAmount.substr(strAmount.length - decimal, 6);
        ...
        return Number('0.'+ dec + strAmount.substr(0, 18))==NaN?0:Number('0.'+ dec + strAmount.substr(0, 18));
      }
    }
  }
